...
BOOST_AUTO_TEST_SUITE(tab)
    ...
    BOOST_AUTO_TEST_CASE(get_control)
    {
        BOOST_TEST_PASSPOINT();

        {
            ...
            BOOST_CHECK(&tab.template get_control<tab_frame_type>() == rp_tab_frame);
        }
        {
            window_type parent{};
            auto p_tab_frame = tetengo2::stdalt::make_unique<tab_frame_type>(parent);
    ...
    BOOST_AUTO_TEST_CASE(visible)
    {
        ...
        BOOST_CHECK_EQUAL(tab.visible(), tab.template get_control<control_type>().visible());
    }
