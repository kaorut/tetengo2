...
namespace tetengo2::gui::widget {
    ...
    */
    ...
    class map_box : public custom_control<DetailsTraits, MouseCaptureDetails>
    {
    public:
        // types
    ...
    private:
        ...
        static void set_observers(map_box& map_box_)
        {
            ...
            map_box_.paint_observer_set().paint_background().connect(
                [&map_box_](canvas_type& canvas) { return map_box_.paint_background(canvas); });
            ...
            map_box_.keyboard_observer_set().key_down().connect(
                [&map_box_](const virtual_key_type& virtual_key, bool, bool, bool) { map_box_.key_down(virtual_key); });
        ...
        bool paint_background(canvas_type& canvas) const
        {
        ...
        std::ptrdiff_t value_count_per_page(const std::ptrdiff_t direction) const
        {
            if (!m_selected_value_index)
                return 0;
        ...
        position_type adjust_position(const position_type& position) const
        {
            ...
            {
                adjusted_top += position_unit_type::from_pixels(this->vertical_scroll_bar().tracking_position()) *
        ...
        scroll_bar_size_type calculate_new_position(const delta_type& delta) const
        {
